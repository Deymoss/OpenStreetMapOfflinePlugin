...
TileProvider::TileProvider() : m_status(Invalid), m_nm(nullptr), m_timestamp(defaultTs)
{

}

TileProvider::TileProvider(const QUrl &urlRedirector)
:   m_status(Idle), m_urlRedirector(urlRedirector), m_nm(nullptr), m_timestamp(defaultTs)
{
...
:   m_status(Invalid), m_nm(nullptr), m_urlTemplate(urlTemplate),
    ...
    m_minimumZoomLevel(minimumZoomLevel), m_maximumZoomLevel(maximumZoomLevel), m_timestamp(defaultTs)
{
    setupProvider();
}
...
void TileProvider::resolveProvider()
{
    ...
    request.setUrl(QUrl("LOL"));//(m_urlRedirector);
